...
import com.bank.event_driven_banking_system.command.idempotency.IdempotencyRecord;
...
import com.bank.event_driven_banking_system.core.events.TransferInitiatedEvent;
...
public class TransferCommandHandler {
    ...
    public String handle(TransferMoneyCommand command) {
        ...
        if (command.getSourceAccountId().equalsIgnoreCase(command.getDestinationAccountId())) {
        ...
        try {
            ...
            // Another request with the same idempotency key was processed at the same time.
            return idempotencyRepository
                    .findByIdempotencyKey(command.getIdempotencyKey())
                    .orElseThrow(() -> new IllegalStateException("Unable to resolve idempotency record."))
                    .getTransferId();
        }
