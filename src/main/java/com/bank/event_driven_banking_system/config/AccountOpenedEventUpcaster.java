...
public class AccountOpenedEventUpcaster extends SingleEventUpcaster {
    ...
    private static final String TARGET_TYPE = "com.bank.event_driven_banking_system.command.events.AccountOpenedEvent";
    private final ObjectMapper objectMapper = new ObjectMapper();

    @Override
    protected boolean canUpcast(IntermediateEventRepresentation intermediateRepresentation) {
        SerializedType type = intermediateRepresentation.getType();
        return TARGET_TYPE.equals(type.getName()) && (type.getRevision() == null || "1.0".equals(type.getRevision()));
    }

    @Override
    protected IntermediateEventRepresentation doUpcast(IntermediateEventRepresentation intermediateRepresentation) {
        return intermediateRepresentation.upcastPayload(
                new SimpleSerializedType(TARGET_TYPE, "2.0"),
                byte[].class,
                bytes -> {
                    try {
